module ap_tools
	...
	! Global collection of variables used by all programes
	integer :: imock0, imock1, num_patch, num_veto, num_vlos=0
	character(len=char_len) :: inputdir, outputdir, patchname, nbarfile, &
		mangledir, Andersonmaskfile, vetomaskfilelist(100), inputfilestr1, inputfilestr2
	real(dl) :: thetaxs(100), thetays(100), thetazs(100), ramin, ramax, decmin, decmax, redmin, redmax, numrat, vlosadds(100), Area_Data, extra_radrop_min, extra_radrop_max
        logical :: only_do_vlosadd = .false., extra_radrop = .false.
...
100		close(1456)
  ...
  !------------------------------------------	
	subroutine read_in (file_name, n_column, n_lines, total_data, skipfirst_ipt)
		! Dummy
		character(len=char_len), intent(in) :: file_name
		integer, intent(in)  :: n_column
		integer, intent(in), optional :: skipfirst_ipt
		integer, intent(out) :: n_lines
		! Local
		character(len=char_len) :: str
		real(dl), allocatable :: total_data(:,:)
		integer :: i, skipfirst=0
		call count_line_number(file_name, n_lines)
		if(present(skipfirst_ipt)) skipfirst = skipfirst_ipt
		n_lines = n_lines - skipfirst
...
33		write(*,*) "Error occurs when writing into the file ", file_name
  ...
  !------------------------------------------  	
	subroutine radecr_to_xyz(ra,dec,r, x,y,z)
		! DUMMY
		real, intent(in) :: ra,dec,r
		real, intent(out) :: x,y,z
		! LOCAL
		real :: ra2, dec2
		real, parameter :: degreefac = const_pi/180.0
		ra2 = ra * degreefac
		dec2 = dec * degreefac
  ...
  !------------------------------------------
  ! get r, theta, phi according to x,y,z. 3D
  !  dl precision version
  !------------------------------------------  	
	subroutine radecr_to_xyz_dlpre(ra,dec,r, x,y,z)
		! DUMMY
		...
		y = r*cos(dec2) * sin(ra2)
		z = r*sin(dec2)
	end subroutine radecr_to_xyz_dlpre
  ...
  ! get r, ra, dec according to x,y,z
  !------------------------------------------
  	subroutine xyz_to_radecr_dlpre(x,y,z,ra,dec,r)
		! DUMMY
		real(dl), intent(in) :: x,y,z
		real(dl), intent(out) :: ra,dec,r
		! LOCAL
		real(dl), parameter :: degreefac = const_pi/180.0
		r = sqrt(x*x+y*y+z*z)
		dec = (const_pi/2.0 - getangle_dlpre(z,sqrt(x*x+y*y))) / degreefac
		ra = getangle_dlpre(x,y) / degreefac
  	end subroutine xyz_to_radecr_dlpre
...
!  		endif
        ...
        ! Some functons used to do rotation
...
!               Matrix = matmul(tmp2,tmp1)
	...
	! read in the information from the inifile
	subroutine get_info_from_ini(inifile)
		! Dummy
		character(len=char_len) :: inifile
		! local
		character(len=char_len) :: tmpstr1
		integer :: i
		
		print *, '######################'
		open(file = inifile,unit=1)

		read(1,*)
		read(1,*) imock0, imock1
		print *, 'imock range = ', imock0, imock1

		read(1,'(A)') inputdir
		print *, 'input  dir: ', trim(adjustl(inputdir))
		read(1,'(A)') outputdir
		print *, 'output dir: ', trim(adjustl(outputdir))

		read(1,'(A)') inputfilestr1
		read(1,'(A)') inputfilestr2
		print *, 'inputfile: str1  = ', trim(adjustl(inputfilestr1))
		print *, 'inputfile: str2  = ', trim(adjustl(inputfilestr2))


		read(1,*) num_patch
		print *, '#-patch = ', num_patch
		do i = 1, num_patch
			read(1,*) thetaxs(i), thetays(i), thetazs(i)
		enddo

		read(1,'(A)') patchname
		print *, 'patchname = ', trim(adjustl(patchname))

		read(1,*) ramin, ramax, decmin, decmax
		print *, 'ra min/max:  ', ramin, ramax
		print *, 'dec min/max:  ', decmin, decmax

		read(1,'(A)') nbarfile
		print *, 'nbarfile = ', trim(adjustl(nbarfile))
	
		read(1,*) numrat
		print *, '#-rat: mock/data = ', numrat
	
		read(1,*) redmin, redmax
		print *, 'redmin/redmax = ', redmin, redmax

		read(1,'(A)') mangledir
		print *, trim(adjustl(mangledir))
		read(1,'(A)') Andersonmaskfile
		print *, trim(adjustl(Andersonmaskfile))
		read(1,*) num_veto
		print *, '#-veto mask:', num_veto
		do i = 1, num_veto
			read(1,'(A)') vetomaskfilelist(i)
			print *, trim(adjustl(vetomaskfilelist(i)))
		enddo
		
		read(1,*,end=900) num_vlos
		do i = 1, num_vlos
			read(1,*) vlosadds(i)
		enddo
                read(1,*) only_do_vlosadd
                print *, 'Only do vlos add: ', only_do_vlosadd
		
		read(1,*) Area_Data
		print *, 'Effective Area of Data: ', Area_Data
		
		read(1,*,end=900) extra_radrop
		print *, 'Extra_radrop, ', extra_radrop
		
		if(extra_radrop) then
			read(1,*) extra_radrop_min, extra_radrop_max
			print *, '   ', extra_radrop_min, extra_radrop_max
		endif
900		continue	
		close(1)
	end subroutine get_info_from_ini
	
	character(len=char_len) function get_inputfilename(imock)
		! Dummy
		integer :: imock
		! local
		character(len=char_len) :: tmpstr1
		write(tmpstr1,*) imock
		if(imock .le. 9) then
		        get_inputfilename = trim(adjustl(inputfilestr1))//'0'//trim(adjustl(tmpstr1))//trim(adjustl(inputfilestr2))
		else                
		        get_inputfilename = trim(adjustl(inputfilestr1))//trim(adjustl(tmpstr1))//trim(adjustl(inputfilestr2))
		endif 
	end function get_inputfilename
end module ap_tools
