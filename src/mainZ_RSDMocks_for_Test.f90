...
use RDistributions
	...
	implicit none
        ...
        integer, parameter :: bignum = 10000000
        integer :: numbins, np, IDs(bignum), nowpatch, numhalo, ivlos, nowtotleftnum, &
...
! Radially-select...        
    ...
    do i_mock = imock0, imock1
        ...
        if(i_mock .le. 9) then
                inputfilename = 'Xiao-dong.0000'//trim(adjustl(tmpstr1))//'.dat.compact' ! Settings !!!
        else                
                inputfilename = 'Xiao-dong.000'//trim(adjustl(tmpstr1))//'.dat.compact' ! Settings !!!
        endif  
...
!		nowRSDoutputfile = trim(adjustl(nowinputfile))//'.RSD-radial-selected'
...
!                write(4, '(A)') '# HR3 mock '//trim(adjustl(tmpstr1))//'. Cosmology: Omegam=0.26,w=-1,h=0.72. Unit: Mpc/h,km/s. Has the same n(z) with DR11v1 # x,y,z,vx,vy,vz,mass,red,ra,dec'
                ...
                do i = 1, np
                        ...
                        write(4,'(5e15.7,i10,3i3)') xs(i)*rats(i), ys(i)*rats(i), zs(i)*rats(i), wfkp(markibin(i) ), masss(i), i,1,1,1
                        write(5,*) ra, dec
                        j = j+1
                enddo
                close(4); close(5)
